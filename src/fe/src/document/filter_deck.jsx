  ...
  return (
    <div className="dropdown dropdown-top dropdown-end absolute left-2 top-0.5 right-[66px]">
      ...
      <ul tabIndex="0" className="dropdown-content block menu menu-compact shadow-md border border-slate-200 bg-base-100 rounded max-h-96 w-72 overflow-auto">
        <li className="block border-b border-slate-100" key={0}><a className={scn.menuA}>New Filter</a></li>
        { Object.keys(filters).map((filter) => (
          <li className="block" key={ filter }><a className={scn.menuA} onClick={ filterClick } data-filter-name={ filter }>{ filter }</a></li>
          ))}
      </ul>
    </div>
  )
  ...
  return (
    <div className="ml-2">
      <div className="h-10 bg-slate-100 items-center w-full">
        ...
        <button className={`w-[60px] absolute top-0 right-0 ${scn.primaryButton}`}>Go</button>
      </div>
